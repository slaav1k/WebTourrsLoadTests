...
public class BuyTicket extends Simulation {
    ...
    private static Map<CharSequence, String> headers_12 = Map.ofEntries(
            Map.entry("Content-Type", "multipart/form-data; boundary=---------------------------56591070230986862232593341303"),
            Map.entry("Origin", "http://localhost:1080"),
            Map.entry("Priority", "u=4")
    );

    // Feeder for random selection of user from CSV
//    private static final FeederBuilder<String> csvFeeder = csv("data/registered_users.csv").random();
...
//                            .check(header("Set-Cookie").saveAs("sessionCookie"))
    ...
    // Invoice
    public static final ChainBuilder invoice = exec(
            group("Invoice").on(
                    http("invoice_0")
                            .post("/cgi-bin/reservations.pl")
                            .headers(headers_12)
                            .body(ElFileBody("0012_request.html"))
                            .check(substring("Thank you for booking through Web Tours"))
                            .check(substring("#{firstName}#{lastName}'s Flight Invoice"))
                            .check(substring("from #{departCity} to #{arriveCity}"))
                            .check(substring("Total Charged to Credit Card #").saveAs("totalCharged"))
    ...
    private static final ChainBuilder debugOutput = exec(
            group("DebugOutput").on(
                    exec(session -> {
                        ...
                        System.out.println("Response for find_flights_0: " + session.getString("responseBodyFrom10"));
                        System.out.println("outboundFlightList: " + session.getList("outboundFlightList"));
                        System.out.println("outboundFlight: " + session.getString("outboundFlight"));
                        System.out.println("Response for payment_0: " + session.getString("responseBodyFrom11"));
                        System.out.println("ticketCost: " + session.getString("ticketCost"));
                        System.out.println("Response for invoice_0: " + session.getString("responseBodyFrom12"));
                        System.out.println("totalCharged: " + session.getString("totalCharged"));
                        System.out.println("Response for logout_0: " + session.getString("responseBodyFrom13"));
                        System.out.println("Response for logout_1: " + session.getString("responseBodyFrom14"));
                        System.out.println("newUserSession: " + session.getString("newUserSession"));
                        System.out.println("User's data: " + session.getString("username") + " , " + session.getString("password"));
                        System.out.println("User's data: " + session.getString("firstName") + " , " + session.getString("lastName") + " , " + session.getString("pass1"));
                        return session;
                    })
            )
    );

//    private ScenarioBuilder scn = scenario("UC_BuyTicket")
//            .feed(csvFeeder)
//            .exec(openSite)
//            .pause(5)
//            .exec(login)
...
//    private static final int COUNT_TICKETS = 5;
    private static final int COUNT_TICKETS = 300;
    ...
    private ScenarioBuilder scn = scenario("UC_BuyTicket")
            ...
            .exec(login)
            .pause(5)
            .repeat(COUNT_TICKETS, "buyTicketsLoop").on(
                    exec(flights)
                            .pause(5)
                            .exec(findFlights)
                            .pause(5)
